...
public final class PolishNotation {
    ...
    public static String convert(Node.Expr expr) {
        return switch (expr) {
            ...
            case Node.Expr.Prefix(var op, var right) -> "(" + op + " " + convert(right) + ")";
            case Node.Expr.Infix(var left, var op, var right) ->
                "(" + op + " " + convert(left) + " " + convert(right) + ")";
        };
    }
}
